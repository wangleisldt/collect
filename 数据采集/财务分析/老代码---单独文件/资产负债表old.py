...
def 获取_全量_资产负债表(stockId):
    ...
    while number_of_none < 5 and year >= 1994 :
        ...
        df = 获取_资产负债表(year, quarter, stockId)
        if df is not None:
            number_of_none = 0
            ...
            #保存文件部分
            try:
                dirname = ct.GLOBAL_PATH + ct.SEPARATOR + ct.FUNDAMENTAL_DATA + ct.SEPARATOR + ct.BalanceSheet + ct.SEPARATOR + str(year)
...
def 获取_全量股票_资产负债表_某个季度(year,quarter):
    ...
    for element in stock_list:
        ...
        dirname = ct.GLOBAL_PATH + ct.SEPARATOR + ct.FUNDAMENTAL_DATA + ct.SEPARATOR + ct.BalanceSheet + ct.SEPARATOR + str(
            year)
...
def 获取_全量股票_资产负债表_某个季度_并行(year,quarter):
    sd_instance = StockDict()
    stock_list = sd_instance.stockIdList
